...
@interface AuditDriverListCell ()

@end

@implementation AuditDriverListCell
#pragma mark 懒加载
- (UIView *)colorLine{
    if (!_colorLine) {
...
- (UIImageView *)icon0{
    if (_icon0 == nil) {
        ...
        _icon0.image = [UIImage imageNamed:@"orderCell_license"];
...
- (UIImageView *)icon1{
    if (_icon1 == nil) {
        ...
        _icon1.image = [UIImage imageNamed:@"orderCell_time"];
...
- (void)resetCellWithModel:(ModelAuditDriver *)model{
    ...
    [self.labelMainTitle fitTitle:isStr(model.realName)?model.realName:@"暂无姓名" fixed:self.labelStatus.left - W(30) - self.colorLine.right];
    self.labelMainTitle.leftCenterY = XY(self.colorLine.right + W(20),self.labelStatus.centerY);
    
    [self.labellSubTitle fitTitle:[NSString stringWithFormat:@"用户账号:%@",UnPackStr(model.account)] fixed:self.viewBG.width - W(30)- self.colorLine.right];
    ...
    [self.label0 fitTitle:isStr(model.idNumber)?(model.idNumber):@"暂无驾驶证号" variable:self.viewBG.width - self.icon0.right - W(40)];
    ...
    [self.label1 fitTitle:UnPackStr(model.timeShow) variable:self.viewBG.width - self.icon1.right - W(40)];
    self.label1.leftCenterY = XY(self.icon1.right + W(11),self.icon1.centerY);
    
    
  
    self.viewBG.height = self.icon1.bottom +W(16);
