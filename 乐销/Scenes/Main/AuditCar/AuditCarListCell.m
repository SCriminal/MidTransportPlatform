...
@interface AuditCarListCell ()

@end

@implementation AuditCarListCell
#pragma mark 懒加载
- (UIView *)colorLine{
    if (!_colorLine) {
...
- (UIImageView *)icon0{
    if (_icon0 == nil) {
        ...
        _icon0.image = [UIImage imageNamed:@"orderCell_company"];
...
- (UIImageView *)icon1{
    if (_icon1 == nil) {
        ...
        _icon1.image = [UIImage imageNamed:@"orderCell_driver"];
...
- (UIImageView *)icon2{
    if (_icon2 == nil) {
        ...
        _icon2.image = [UIImage imageNamed:@"orderCell_license"];
...
- (void)resetCellWithModel:(ModelAuditCar *)model{
    ...
    [self.label0 fitTitle:UnPackStr(model.entName) variable:self.viewBG.width - self.icon0.right - W(40)];
    ...
    [self.label1 fitTitle:isStr(model.driverName)?model.driverName:@"暂无司机"  variable:self.viewBG.width - self.icon1.right - W(40)];
    ...
    [self.label2 fitTitle:UnPackStr(model.vehicleLicense) variable:self.viewBG.width - self.icon2.right - W(40)];
    self.label2.leftCenterY = XY(self.icon2.right + W(11),self.icon2.centerY);
    
    self.icon3.leftTop = XY(self.icon2.left,self.icon2.bottom + W(10));
    [self.label3 fitTitle:model.timeShow variable:self.viewBG.width - self.icon3.right - W(40)];
    self.label3.leftCenterY = XY(self.icon3.right + W(11),self.icon3.centerY);
    
    self.viewBG.height = self.icon3.bottom +W(16);
